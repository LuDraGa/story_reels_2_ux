...
This file provides guidance to Codex CLI (GPT-5) when working in this repository.

## Quick Reference

- `docs/PROJECT_PROMPTS.md`: Original prompts and requirements for each build phase (essential for new sessions)
- `docs/ARCHITECTURE.md`: Tech stack, routes, database schema, Supabase setup
- `docs/DESIGN_SYSTEM.md`: 60-30-10 colors, typography, spacing (edit one file to change all)
- `docs/API_INTEGRATION.md`: Modal Coqui TTS API integration (CRITICAL - external API)
- `docs/WORKFLOWS.md`: Pipeline modules, job queue, common patterns, testing
- `docs/DEPLOYMENT.md`: Vercel deployment, GitHub Actions heartbeat
- `docs/SUPABASE_MIGRATIONS.md`: Database migrations, CLI commands, troubleshooting
- `docs/HEARTBEAT.md`: GitHub Actions keep-alive to prevent Supabase Free tier pause
- `docs/CODE_SEARCH.md`: Code search tools and strategies
- `docs/HANDOFF_GUIDE.md`: How to create ideal handoff documents
- `execution_docs/`: Task planning and execution tracking
- `HANDOFF_PHASE_2.md`: Phase 1→2 handoff (example of ideal handoff doc)
...
- **Integration only** - see `docs/API_INTEGRATION.md`
...
**Migration Commands**: See `docs/SUPABASE_MIGRATIONS.md` for CLI commands

**Schema Details**: See `docs/ARCHITECTURE.md` for full schema
...
See `docs/WORKFLOWS.md` for UI patterns.
...
### ✅ DO
- Read `docs/API_INTEGRATION.md` first before touching TTS/voice code
...
**Starting a new session?** Check `docs/PROJECT_PROMPTS.md` first to understand original requirements and project history.
...
See `docs/WORKFLOWS.md` for detailed patterns.
